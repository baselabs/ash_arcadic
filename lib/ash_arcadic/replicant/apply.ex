# Compile-gated on the optional `replicant` dep (usage-rules.md, "Optional-dep"): this module
# pattern-matches `%Replicant.Transaction{}` / `%Replicant.Change{}` structs, so it compiles only
# when `replicant` is present. A non-CDC host builds ash_arcadic without it — the whole
# `AshArcadic.Replicant.*` apply/impl/pipeline subtree compiles away. Keep the guard.
